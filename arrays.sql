...
begin
	...
	-- Связи
	alter table if exists artyomg.measurment_input_params
	drop constraint if exists measurment_type_id_fk;

	alter table if exists artyomg.employees
	drop constraint if exists military_rank_id_fk;

	alter table if exists artyomg.measurment_baths
	...
	-- Таблицы
	drop table if exists artyomg.measurment_input_params;
	drop table if exists artyomg.measurment_baths;
	drop table if exists artyomg.employees;
	drop table if exists artyomg.measurment_types;
	drop table if exists artyomg.military_ranks;
	drop table if exists artyomg.measurment_settings;

	-- Нумераторы
	drop sequence if exists artyomg.measurment_input_params_seq;
	drop sequence if exists artyomg.measurment_baths_seq;
	drop sequence if exists artyomg.employees_seq;
	drop sequence if exists artyomg.military_ranks_seq;
	drop sequence if exists artyomg.measurment_types_seq;
end;
...
create table artyomg.military_ranks
(
	id integer primary key not null,
	description character varying(255)
);

insert into artyomg.military_ranks(id, description)
values(1,'Рядовой'),(2,'Лейтенант');

create sequence artyomg.military_ranks_seq start 3;

alter table artyomg.military_ranks alter column id set default nextval('artyomg.military_ranks_seq');

-- Пользователя
create table artyomg.employees
(
    id integer primary key not null,
	name text,
	birthday timestamp ,
...
insert into artyomg.employees(id, name, birthday,military_rank_id )  
values(1, 'Воловиков Александр Сергеевич','1978-06-24', 2);

create sequence artyomg.employees_seq start 2;

alter table artyomg.employees alter column id set default nextval('artyomg.employees_seq');


-- Устройства для измерения
create table artyomg.measurment_types
(
   id integer primary key not null,
...
insert into artyomg.measurment_types(id, short_name, description)
values(1, 'ДМК', 'Десантный метео комплекс'),
(2,'ВР','Ветровое ружье');

create sequence artyomg.measurment_types_seq start 3;

alter table artyomg.measurment_types alter column id set default nextval('artyomg.measurment_types_seq');

-- Таблица с параметрами
create table artyomg.measurment_input_params 
(
    id integer primary key not null,
...
insert into artyomg.measurment_input_params(id, measurment_type_id, height, temperature, pressure, wind_direction,wind_speed )
values(1, 1, 100,12,34,0.2,45);

create sequence artyomg.measurment_input_params_seq start 2;

alter table artyomg.measurment_input_params alter column id set default nextval('artyomg.measurment_input_params_seq');

-- Таблица с историей
create table artyomg.measurment_baths
(
		id integer primary key not null,
...
insert into artyomg.measurment_baths(id, emploee_id, measurment_input_param_id)
values(1, 1, 1);

create sequence artyomg.measurment_baths_seq start 2;

alter table artyomg.measurment_baths alter column id set default nextval('artyomg.measurment_baths_seq');

-- Таблица с настройками
create table artyomg.measurment_settings
(
...
insert into artyomg.measurment_settings(key, value, description)
...
drop table if exists artyomg.calc_temperatures_correction;
create table artyomg.calc_temperatures_correction
(
   temperature numeric(8,2) primary key,
   correction numeric(8,2)
);

insert into artyomg.calc_temperatures_correction(temperature, correction)
Values(0, 0.5),(5, 0.5),(10, 1), (20,1), (25, 2), (30, 3.5), (40, 4.5);

drop type  if exists artyomg.interpolation_type;
create type artyomg.interpolation_type as
(
	x0 numeric(8,2),
	x1 numeric(8,2),
	y0 numeric(8,2),
...
drop type if exists artyomg.input_params cascade;
create type artyomg.input_params as
(
	height numeric(8,2),
...
begin 
	...
	alter table artyomg.measurment_baths
	add constraint emploee_id_fk 
	foreign key (emploee_id)
	references artyomg.employees (id);
	
	alter table artyomg.measurment_baths
	add constraint measurment_input_param_id_fk 
	foreign key(measurment_input_param_id)
	references artyomg.measurment_input_params(id);
	
	alter table artyomg.measurment_input_params
	add constraint measurment_type_id_fk
	foreign key(measurment_type_id)
	references artyomg.measurment_types (id);
	
	alter table artyomg.employees
	add constraint military_rank_id_fk
	foreign key(military_rank_id)
	references artyomg.military_ranks (id);
...
drop function if exists   artyomg.fn_calc_header_temperature;
create function artyomg.fn_calc_header_temperature(
...
begin	
	...
	from artyomg.measurment_settings 
	...
	deltaTv := par_temperature + 
		artyomg.fn_calc_temperature_interpolation(par_temperature => par_temperature);
...
drop function if exists artyomg.fn_calc_header_period;
create function artyomg.fn_calc_header_period(
...
drop function if exists artyomg.fn_calc_header_pressure;
create function artyomg.fn_calc_header_pressure
(
...
begin
	...
	if not exists (select 1 from artyomg.measurment_settings where key = default_pressure_key ) then
	Begin
	...
	begin
		...
		into table_pressure
		from  artyomg.measurment_settings where key = default_pressure_key;
	end;
	end if;
...
drop function if exists artyomg.fn_check_input_params(numeric(8,2), numeric(8,2),   numeric(8,2), numeric(8,2), numeric(8,2), numeric(8,2));
create function artyomg.fn_check_input_params(
...
returns artyomg.input_params
language 'plpgsql'
as $body$
declare
	var_result artyomg.input_params;
begin
	...
	if not exists (
		select 1 from (
				...
				(select 1 ) as t
					cross join
					( select value as  min_temperature from artyomg.measurment_settings where key = 'min_temperature' ) as t1
					cross join 
					( select value as  max_temperature from artyomg.measurment_settings where key = 'max_temperature' ) as t2
				) as t	
			where
	...
	if not exists (
		select 1 from (
			...
			(select 1 ) as t
				cross join
				( select value as  min_pressure from artyomg.measurment_settings where key = 'min_pressure' ) as t1
				cross join 
				( select value as  max_pressure from artyomg.measurment_settings where key = 'max_pressure' ) as t2
			) as t	
			where
	...
	var_result.pressure = par_pressure;			
		...
		if not exists (
			select 1 from (
				...
				(select 1 ) as t
					cross join
					( select value as  min_height from artyomg.measurment_settings where key = 'min_height' ) as t1
					cross join 
					( select value as  max_height from artyomg.measurment_settings where key = 'max_height' ) as t2
				) as t	
				where
		...
		if not exists (
			select 1 from (	
				...
				(select 1 ) as t
					cross join
					( select value as  min_wind_direction from artyomg.measurment_settings where key = 'min_wind_direction' ) as t1
					cross join 
					( select value as  max_wind_direction from artyomg.measurment_settings where key = 'max_wind_direction' ) as t2
			)
				where
...
drop function if exists artyomg.fn_check_input_params(input_params);
create function artyomg.fn_check_input_params(
	par_param artyomg.input_params
)
returns artyomg.input_params
language 'plpgsql'
as $body$
...
drop function if exists artyomg.fn_calc_temperature_interpolation;
create function artyomg.fn_calc_temperature_interpolation(
		par_temperature numeric(8,2))
		returns numeric
		language 'plpgsql'
as $body$
	-- Расчет интерполяции 
	declare 
			var_interpolation artyomg.interpolation_type;
	        var_result numeric(8,2) default 0;
	...
	begin
                ...
                if exists (select 1 from artyomg.calc_temperatures_correction where temperature = par_temperature ) then
                begin
                        select correction 
                        into  var_result 
                        from  artyomg.calc_temperatures_correction
                        where 
                ...
                begin
                        ...
                        (
                                ...
                                from artyomg.calc_temperatures_correction as t1
...
drop function if exists artyomg.fn_get_random_timestamp;
create function artyomg.fn_get_random_timestamp(
...
drop function if exists artyomg.fn_get_randon_integer;
create function artyomg.fn_get_randon_integer(
...
drop function if exists artyomg.fn_get_random_text;
create function artyomg.fn_get_random_text(
   par_length int,
...
begin
    ...
    for var_position in 1 .. par_length loop
	    ...
	    var_random_number := artyomg.fn_get_randon_integer(var_min_value, var_max_value );
...
drop function if exists artyomg.fn_calc_header_meteo_avg;
create function artyomg.fn_calc_header_meteo_avg(
	par_params artyomg.input_params
)
returns text
language 'plpgsql'
as $body$
declare
	var_result text;
	var_params artyomg.input_params;
begin

	-- Проверяю аргументы
	var_params := artyomg.fn_check_input_params(par_params);
	
	select
		-- Дата
		artyomg.fn_calc_header_period(now()) ||
...
begin
	...
	var_pressure_value :=  artyomg.fn_calc_header_pressure(743);
	var_temperature_value := artyomg.fn_calc_header_temperature(23);
	
	select
		-- Дата
		artyomg.fn_calc_header_period(now()) as "ДДЧЧМ",
		--Высота расположения метеопоста над уровнем моря.
...
begin
	...
	from artyomg.military_ranks;

	
	-- Формируем список пользователей
	for var_position in 1 .. var_emploee_quantity loop
		insert into artyomg.employees(name, birthday, military_rank_id )
		select 
			artyomg.fn_get_random_text(25),								-- name
			artyomg.fn_get_random_timestamp('1978-01-01','2000-01-01'), 				-- birthday
			artyomg.fn_get_randon_integer(var_min_rank, var_max_rank)  -- military_rank_id
			;
		select id into var_emploee_id from artyomg.employees order by id desc limit 1;	
	...
	foreach var_current_emploee_id in ARRAY var_emploee_ids LOOP
		for var_index in 1 .. 100 loop
			var_measure_type_id := artyomg.fn_get_randon_integer(1,2);
			
			insert into artyomg.measurment_input_params(measurment_type_id, height, temperature, pressure, wind_direction, wind_speed)
			select
				var_measure_type_id,
				artyomg.fn_get_randon_integer(0,600)::numeric(8,2), -- height
				artyomg.fn_get_randon_integer(0, 50)::numeric(8,2), -- temperature
				artyomg.fn_get_randon_integer(500, 850)::numeric(8,2), -- pressure
				artyomg.fn_get_randon_integer(0,59)::numeric(8,2), -- ind_direction
				artyomg.fn_get_randon_integer(0,59)::numeric(8,2)	-- wind_speed
				;

			select id into var_measure_input_data_id from artyomg.measurment_input_params order by id desc limit 1;
				
			insert into artyomg.measurment_baths( emploee_id, measurment_input_param_id, started)
			select
				var_current_emploee_id,
				var_measure_input_data_id,
				artyomg.fn_get_random_timestamp('2025-02-01 00:00', '2025-02-05 00:00')
			;	
		end loop;
...
CREATE OR REPLACE FUNCTION artyomg.calculate_temperature_deviation(temp_values numeric[])
RETURNS numeric[] AS $$
DECLARE
    avg_temp numeric;
    deviations numeric[] := '{}';
    i integer;
BEGIN
    SELECT avg(t) INTO avg_temp FROM unnest(temp_values) AS t;
    
    FOR i IN 1..array_length(temp_values, 1) LOOP
        deviations := array_append(deviations, temp_values[i] - avg_temp);
    END LOOP;
...
SELECT artyomg.calculate_temperature_deviation(ARRAY[15.2, 17.8, 19.5, 16.1, 18.3]);
...
SELECT 
    e.full_name AS "ФИО", 
    r.rank_name AS "Должность", 
    COUNT(mb.id) AS "Кол-во измерений",
    SUM(CASE 
        WHEN mip.temperature NOT BETWEEN -50 AND 50 
          OR mip.pressure NOT BETWEEN 500 AND 1100 
          OR mip.wind_speed < 0 
        THEN 1 ELSE 0 
    END) AS "Количество ошибочных данных"
FROM employees e
JOIN military_ranks r ON e.military_rank_id = r.id
LEFT JOIN measurment_baths mb ON e.id = mb.emploee_id
LEFT JOIN artyomg.measurment_input_params mip ON mb.measurment_input_param_id = mip.id
GROUP BY e.id, r.rank_name
...
CREATE TABLE artyomg.employees (
    ...
    military_rank_id INT NOT NULL REFERENCES military_ranks(id)
);
...
CREATE TABLE artyomg.measurment_baths (
    id SERIAL PRIMARY KEY,
    emploee_id INT NOT NULL REFERENCES employees(id),
    measurment_input_param_id INT NOT NULL REFERENCES artyomg.measurment_input_params(id),
    started TIMESTAMP DEFAULT NOW()
);

INSERT INTO artyomg.military_ranks (rank_name) VALUES ('Сержант'), ('Лейтенант'), ('Капитан');

INSERT INTO artyomg.employees (full_name, military_rank_id) VALUES 
('Иванов Иван Иванович', 1),
('Петров Петр Петрович', 2),
('Сидоров Сидор Сидорович', 3);

INSERT INTO artyomg.measurment_input_params (temperature, pressure, wind_speed) VALUES 
(15.5, 1010, 5),
(22.3, 1005, 8),
(-5.2, 990, 12),
(30.1, 1020, 3),
...
INSERT INTO artyomg.measurment_baths (emploee_id, measurment_input_param_id) VALUES 
(1, 1), (1, 2), (2, 3), (2, 4), (3, 5);
...
BEGIN
    ...
    FOR i IN 1..ARRAY_LENGTH(temp_values, 1) LOOP
        deviations[i] := temp_values[i] - avg_temp;
    END LOOP;
...
SELECT artyomg.calculate_temperature_deviation(ARRAY[15.5, 22.3, -5.2, 30.1, 17.8]);
